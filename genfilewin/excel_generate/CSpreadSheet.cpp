...
m_bAppend(false), m_bBackup(Backup), m_bTransaction(false)
{
	...
	if (m_stempString == ".xls" || m_stempString == "xlsx" ) //|| m_stempString == ".csv") // File is an Excel spreadsheet
	{
		m_bExcel = true;
...
bool CSpreadSheet::InitFile(PVOID paras)
{
	return true;
}

// Add header row to spreadsheet
bool CSpreadSheet::AddHeaders(CStringArray &FieldNames, std::vector<int>* pDatatype, bool replace)
{
	if (pDatatype)
	{
...
bool CSpreadSheet::AddRows(CStringArray &RowValues, long len, long beginAt, bool replace)
{
...
bool CSpreadSheet::AddRow(CStringArray &RowValues, long row, bool replace)
{
	...
	if (row == 1)
	{
		ReadRow(m_headerArray, 1);
	}

}
catch (...)
{
	CString str = GetLastError();
	::MessageBox(NULL, str.GetBuffer(), "error", MB_OK);
}
...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		//CString strSQL;
		//m_stempSql.Format("INSERT INTO [%s$] (", m_sSheetName);
		//for (int i = 0; i < m_aFieldNames.GetSize(); i++)
		//{
		//	m_stempString.Format("[%s], ", m_aFieldNames.GetAt(i));
		//	strSQL += m_stempString;
		//}
		...
		// Save changed data
		CString strValue;
		for (int k = m_beginAt; k < m_dTotalRows; k++)
		{
			ReadRow(m_atempArray, k+1);
			...
			for (int j = 0; j < m_atempArray.GetSize(); j++)
			{
				strValue = m_atempArray.GetAt(j);
				switch (m_datatype[j])
				{
				case DINT:
					{
						int iValue = atoi(strValue.GetBuffer());
						m_stempString.Format("'%d', ", iValue);
						break;
					}
				case DDOUBLE:
					{
						double dValue = atof(strValue.GetBuffer());
						m_stempString.Format("'%f', ", dValue);
						break;
					}
				case DDATE:
				/*	{
				...
				default:
					{
						m_stempString.Format("'%s', ", strValue);
					}
				}
				m_stempSql += m_stempString;
			}
			m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
			m_stempSql += ")";
...
bool CSpreadSheet::Open()
{
	...
	{
		m_Database->OpenEx(m_sDsn, CDatabase::noOdbcDialog);
		CString strTableName = m_Database->GetDatabaseName();
		...
		//m_rSheet->GetFieldValue(1, strTableName);
		
		m_sSql.Format("SELECT * FROM [%s$A1:IV65536]", m_sSheetName);
		try
		{
			m_rSheet->Open(CRecordset::forwardOnly, /*_bstr_t(*/m_sSql, CRecordset::readOnly);
		}
		catch(...)
		{
			delete m_rSheet;
